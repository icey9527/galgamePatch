...
class TileEncoder:
    ...
    def _render_glyph_to_gray_tile(self, ch: str) -> bytearray:
        ...
        dst_x0 = int(slot.bitmap_left) + self._offset_x
        ...
        dst_y0 = self._baseline_y - int(slot.bitmap_top) + self._offset_y
    ...
    def render_text(self, text: str) -> bytes:
        ...
        # 原始灰度字形
        src = self._render_glyph_to_gray_tile(text[0])
        tile_gray = bytearray(src)

        # 第一步：找“字形边缘像素”
        edge = bytearray(len(src))
        for y in range(self.tile_h):
            for x in range(self.tile_w):
                ...
                for nx, ny in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)):
                    if 0 <= nx < self.tile_w and 0 <= ny < self.tile_h:
                        if src[ny * self.tile_w + nx] == 0:
                            edge[i] = 1
                            break

        # 第二步：仅从“边缘像素”向外扩一圈灰色描边
        outline_value = 68  # 灰色描边，可试 80/96/112
        for y in range(self.tile_h):
            for x in range(self.tile_w):
                i = y * self.tile_w + x
                if src[i] != 0:
                ...
                for nx, ny in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1),
                    ...
                    if 0 <= nx < self.tile_w and 0 <= ny < self.tile_h:
                        if edge[ny * self.tile_w + nx]:
                            near_edge = True
                            break

                if near_edge:
                    tile_gray[i] = outline_value

        # 第三步：原字盖回去，保证主体不被描边影响
